  ...
  describe('KataBankOCR (parser: ' + parser.name + ')', function() {
    describe('User Story 1 - parsing', function() {
      accounts.forEach(function(account, i) {
        it('should parse account number ' + account, function() {
          equalNumber(account, 'test1_' + i + '.txt', parser);
        });
      });
    ...
    describe('User Story 4 - fix errors', function() {
      ...
      it('should resolve an exact account number for 111111111', function() {
        equalResolve('711111111', 'test4_1.txt', parser);
      });
      it('should resolve an exact account number for 777777777', function() {
        equalResolve('777777177', 'test4_2.txt', parser);
      });
      it('should resolve an exact account number for 200000000', function() {
        equalResolve('200800000', 'test4_3.txt', parser);
      });
      it('should resolve an exact account number for 333333333', function() {
        equalResolve('333393333', 'test4_4.txt', parser);
      });
      it('should resolve ambiguously for 888888888', function() {
        equalResolve("888888888 AMB ['888886888', '888888880', '888888988']", 'test4_5.txt', parser);
      });
      it('should resolve ambiguously for 555555555', function() {
        equalResolve("555555555 AMB ['555655555', '559555555']", 'test4_6.txt', parser);
      });
      it('should resolve ambiguously for 666666666', function() {
        equalResolve("666666666 AMB ['666566666', '686666666']", 'test4_7.txt', parser);
      });
      it('should resolve ambiguously for 999999999', function() {
        equalResolve("999999999 AMB ['899999999', '993999999', '999959999']", 'test4_8.txt', parser);
      });
      it('should resolve ambiguously for 490067715', function() {
        equalResolve("490067715 AMB ['490067115', '490067719', '490867715']", 'test4_9.txt', parser);
      });
      it('should resolve an exact account number for 123456789', function() {
        equalResolve('123456789', 'test4_10.txt', parser);
      });
      it('should resolve an exact account number for 000000051', function() {
        equalResolve('000000051', 'test4_11.txt', parser);
      });
      it('should resolve an exact account number for 490867715', function() {
        equalResolve('490867715', 'test4_12.txt', parser);
      });
    });
    describe('User Story 5 - multiple fixes', function() {
      it('should resolve two ambiguous account numbers for ?9086717?, limit 2', function() {
        equalResolveMultiple("?9086717? AMB ['490857175', '490967175']", 'test5_1.txt', parser, 2);
      });
      it('should resolve two ambiguous account numbers for 1234?678?, limit 3', function() {
        equalResolveMultiple("1234?678? AMB ['123456789', '723436789']", 'test5_2.txt', parser, 3);
      });
      it('should resolve an exact account number 1234?678?, limit 2', function() {
        equalResolveMultiple("123456789", 'test5_3.txt', parser, 2);
      });
    });
  });
});
...
function equalNumber(account, filename, parser) {
  assert.equal(account, parser.parseFile('test/' + filename)[0].number);
}

function equalStatus(account, filename, parser) {
  assert.equal(account, print(addStatus(parser.parseFile('test/' + filename)))[0]);
}

function equalResolve(account, filename, parser) {
  assert.equal(account, print(resolve(addStatus(parser.parseFile('test/' + filename))))[0]);
}

function equalResolveMultiple(account, filename, parser, max) {
  assert.equal(account, print(resolveMultiple(addStatus(parser.parseFile('test/' + filename)), max))[0]);
}